...
$sql = " Select * from (";
$sql .= "SELECT 1 as tipo,t1.id_equipo,t1.id_manipulador,fecha_alta,fecha_ultimo_acceso,activa,t2.Nombre_Comercial,cif ";
$sql .= " FROM mds_licencias_software as t1 ";
$sql .= " left join mds_manipuladores as t2 on t1.id_manipulador=t2.id_manipulador";
$sql .= " UNION ";
$sql .= " SELECT 2 as tipo,t1.id_equipo,t1.id_manipulador,fecha_alta,fecha_ultimo_acceso,activa,t2.Nombre_Comercial,cif ";
$sql .= " FROM mds_licencias_software_etiamb2 as t1 ";
$sql .= " left join mds_manipuladores as t2 on t1.id_manipulador=t2.id_manipulador ";
$sql .= " ) as todo";
...
if ($tipo !=0)
{
    if ($tipo ==1)
    {
        $where = $where." and todo.tipo=1";
    }
    elseif($tipo==2)
    {
        $where = $where." and todo.tipo=2";
    }    
}
...
if ($activo !=0)
{
    if ($activo ==1)
    {
        $where = $where." and todo.activa=1";
    }
    elseif($activo==2)
    {
        $where = $where." and todo.activa=0";
    }    
}
if ($nombrecomercial!="")
{
  $where = $where." and todo.Nombre_Comercial like '%".$nombrecomercial."%'" ;
}
if ($cif !="")
{
  $where = $where." and todo.cif like '%".$cif."%'" ;
}
if ($where!="")
{
...
{
    //Cojo el primer digito
    var tipo = element.id.substring(0, 1);
    //Cojo del 7 digito en adelante
    var id_equipo = element.id.substring(7, element.id.lengh);
    var activo = 0;
...
<table id="datatable-buttons" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
	...
	<?php while ($rs = mysqli_fetch_array($resultado)) { ?>
      ...
      <td>
        <input type="checkbox" id="<?php echo $rs['tipo'];?>activa<?php echo $rs['id_equipo'];?>" class="checkbox js-switch" name="activa<?php echo $rs['id_equipo'];?>" value="1" onchange="actualizar_activo(this)" <?php echo ($rs['activa']==1 ? 'checked' : 'unchecked');?>/>
      </td>
